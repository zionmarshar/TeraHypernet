...
fun sendTerabitData(ctx: ChannelHandlerContext, terabit: TerabitData) {
    try {
        val data = base64Enc(terabit.encode())
        sendJson(ctx, """{"code":0,"data":"$data"}""")
        return
    } catch (e: Exception) {
        sendJsonCode(ctx, 100, e.message ?: "Exception")
    }
}
...
fun sendObjectAsJson(ctx: ChannelHandlerContext,obj:Any?){
    sendJsonObject(ctx, Gson().toJson(obj))
}
fun sendJsonObject(
    ctx: ChannelHandlerContext,
    jsonObj: String,
